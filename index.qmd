---
title: "Teaching Materials"
subtitle: "Quantitative Genetics, Genomics, and Statistical Modeling"
toc: false
---

This site hosts structured course materials, lecture notes, slides, and computational demonstrations in quantitative genetics, genomics, and statistical modeling. All materials are reproducible and developed using R and Quarto.

---

## 🎓 Courses

::: {.columns}

::: {.column width="48%"}

### 📘 Quantitative & Population Genetics

This module introduces the principles of population and quantitative genetics, including genetic variation, Hardy–Weinberg equilibrium, inbreeding, mutation, recombination, linkage disequilibrium, genetic drift, and natural selection. It also presents statistical approaches for analyzing complex traits, including heritability estimation, phenotype prediction, and genome-wide association studies (GWAS).

[Go to Course →](quant-genetics/)

:::

::: {.column width="48%"}

### 🧬 Genomics, Systems Biology & Bioinformatics

This module explores how genomic and multi-omics data can be used to understand complex biological systems. Topics include sequencing technologies, integration of molecular data, genome-wide association studies (GWAS), network models, and machine learning approaches for analyzing biological data.

[Go to Course →](genomics-systems-bioinfo/)

:::

:::

::: {.columns}

::: {.column width="50%"}

### 📊 Bayesian Linear Regression

This module introduces Bayesian linear regression through theoretical notes, slides, and practical R examples. It covers prior specification, posterior inference using Gibbs sampling, and model interpretation, with emphasis on practical implementation and convergence assessment.

[Go to Course →](https://psoerensen.github.io/bayesian-linear-regression/)

:::

::: {.column width="50%"}

<!-- future module -->

:::

:::


---

## 💻 Teaching Philosophy

The material emphasizes:

- Mathematical foundations  
- Statistical modeling  
- Reproducible research  
- Computational implementation in R  
- Conceptual understanding of methods  

Lecture materials integrate theory, simulation, and applied examples.

---

## The *qgg* R Package

[**qgg**](https://github.com/psoerensen/qgg) provides tools for statistical modeling and analysis of large-scale genomic data, including:

- Fine-mapping of genomic regions using **Bayesian Linear Regression (BLR) models**  
- Polygenic scoring using **Bayesian Linear Regression (BLR) models**  
- Gene set enrichment analysis using **Bayesian Linear Regression (BLR) models**  

*qgg* handles large-scale genomic data through **efficient algorithms** and **sparse matrix techniques**, combined with **multi-core processing** using **OpenMP**, **multithreaded matrix operations** via **BLAS** libraries (e.g., OpenBLAS, ATLAS, or MKL), and **fast, memory-efficient batch processing** of genotype data stored in  
binary formats such as **PLINK `.bed` files**.


## The *gact* R Package

[**gact**](https://github.com/psoerensen/gact) provides an infrastructure for efficient processing of large-scale genomic association data, with core functions for:

- Establishing and populating a database of genomic associations  
- Downloading and processing biological databases  
- Handling and processing GWAS summary statistics  
- Linking genetic markers to genes, proteins, metabolites, and biological pathways  
- Integrates with statistical machine learning tools in the [**qgg**](https://github.com/psoerensen/qgg) R package  

*gact* is intended to serve as **a practical implementation of integrative genomics**, bridging **statistical modeling** and **biological interpretation**, and supporting **reproducible** and **extensible** workflows.

[**Slides introducing the *qgg* and *gact* R packages**](https://psoerensen.github.io/bayesian-linear-regression/introduction_gact_qgg_slides.html)

[**Tutorials using the *qgg* and *gact* R packages**](https://psoerensen.github.io/gact/)

---

---

## 🧰 Software

::: {.columns}

::: {.column width="50%"}

### The *qgg* R Package

[**qgg**](https://github.com/psoerensen/qgg) provides tools for statistical modeling and analysis of large-scale genomic data, including:

- Fine-mapping of genomic regions using **Bayesian Linear Regression (BLR) models**  
- Polygenic scoring using **Bayesian Linear Regression (BLR) models**  
- Gene set enrichment analysis using **Bayesian Linear Regression (BLR) models**

*qgg* is designed for large-scale genomic analyses using **efficient algorithms**, **sparse matrix techniques**, and **parallel computing**. It supports **multi-core processing** via **OpenMP**, multithreaded matrix operations through **BLAS libraries** (e.g., OpenBLAS, ATLAS, or MKL), and memory-efficient batch processing of genotype data stored in binary formats such as **PLINK `.bed` files**.

:::

::: {.column width="50%"}

### The *gact* R Package

[**gact**](https://github.com/psoerensen/gact) provides an infrastructure for efficient processing and integration of large-scale genomic association data. Core functionality includes:

- Establishing and populating a database of genomic associations  
- Downloading and processing biological databases  
- Handling and processing GWAS summary statistics  
- Linking genetic markers to genes, proteins, metabolites, and biological pathways  
- Integration with statistical modeling tools in the [**qgg**](https://github.com/psoerensen/qgg) R package

*gact* supports **integrative genomics workflows**, connecting **statistical modeling** with **biological interpretation** while enabling **reproducible** and **extensible** analyses.

:::

:::

### Additional Resources

- [**Slides introducing the *qgg* and *gact* R packages**](https://psoerensen.github.io/bayesian-linear-regression/introduction_gact_qgg_slides.html)

- [**Tutorials using the *qgg* and *gact* R packages**](https://psoerensen.github.io/gact/)

---

## 📚 Further Reading

For foundational principles of classical and molecular genetics:

**[Griffiths, A. J. F., Doebley, J. F., Peichel, C. L., & Wassarman, D. A. (2023). *An Introduction to Genetic Analysis* (13th ed.). W. H. Freeman.](https://store.macmillanlearning.com/us/product/Introduction-to-Genetic-Analysis-Digital-Update/p/1319525199)**  


For a unified and modern treatment of quantitative genetics theory and genomics:

**[Walsh, B., Visscher, P. M., & Lynch, M. (2025). *Genetics and Analysis of Quantitative Traits: Foundations* (2nd ed.). Cambridge University Press.](https://global.oup.com/academic/product/genetics-and-analysis-of-quantitative-traits-9780192898180?cc=dk&lang=en&)**  

For Bayesian modeling, hierarchical inference, and statistical learning in quantitative genetics and genomics:

**[Sorensen, D. (2025). *Statistical Learning in Genetics: An Introduction Using R*. Springer.](https://link.springer.com/book/10.1007/978-3-031-86274-8)**  


For likelihood-based and Bayesian inference, including detailed treatment of MCMC methods:

**[Sorensen, D., & Gianola, D. (2002/2010). *Likelihood, Bayesian, and MCMC Methods in Quantitative Genetics*. Springer.](https://link.springer.com/book/10.1007/b98952)**  


For a comprehensive and mathematically rigorous treatment of modern statistical learning:

**[Hastie, T., Tibshirani, R., & Friedman, J. (2009). *The Elements of Statistical Learning: Data Mining, Inference, and Prediction* (2nd ed.). Springer.](https://link.springer.com/book/10.1007/978-0-387-84858-7)**  

For an accessible and application-oriented introduction to statistical learning with R:

**[James, G., Witten, D., Hastie, T., & Tibshirani, R. (2021). *An Introduction to Statistical Learning: With Applications in R* (2nd ed.). Springer.](https://link.springer.com/book/10.1007/978-1-0716-1418-1)**  


## 📫 Contact

Peter Sørensen  
Center for Quantitative Genetics and Genomics  
Department of Molecular Biology and Genetics  
Aarhus University, Denmark  

Email: pso@qgg.au.dk  
GitHub: https://github.com/psoerensen
